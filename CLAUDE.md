...
- **O0 is library-default; Ofast is CLI-default (mforth-10t.40, decision `mforth-ump` Option A).** The orchestrator `src/mforth/optimize.py` (`compile_text(opt_level=...)`) defaults to `OptLevel.O0` so the STRICT teaching-equivalence harness stays byte/event-identical to the REPL; only `mforth compile` defaults to `-Ofast`. CSE/LICM legitimately elide redundant `@` fetches, so optimized levels emit fewer `VariableRead`/`VariableWrite` instrumentation events than the REPL — that's a deliberate, correct divergence. Optimized levels are validated by SINK-event equivalence (print/control/printflush/wait/sensor identical; instrumentation counts allowed to differ): `tests/integration/test_optimized_equivalence.py` + the benchmark harness `tests/bench/`. See `docs/reference/optimization-levels.md`.
